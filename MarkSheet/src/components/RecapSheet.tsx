            ...
            <tbody>
              ...
              <tr>
                    ...
                    onChange={e => {
                      ...
                      setStudentEx({ ...studentEx,
                        marks: studentEx.marks.map(mm => mm.head.headname === m.head.headname
                          ? { ...mm, marks: e.target.value.length === 0 ? '' : e.target.value }
                          : mm) });
                    }} />
                      </td>
                  </tr>
                ...
                onClick={e => {
                  ...
                  if (!dbData) return;
                  setDbData({ grades: dbData.grades, heads: dbData.heads, students: dbData.students.map((s) => s.regno !== studentEx.regno ? s : studentEx) as (Student & { total: number; grade: string; })[] });
